...
//        // similar result user query
		...
		var advisor = RetrievalAugmentationAdvisor.builder()
				...
				.queryAugmenter(ContextualQueryAugmenter.builder().allowEmptyContext(true).build()).build();

		return chatClient.prompt().advisors(advisor)
...
//                                .build())
				...
				user.text(userMessage).param("query", query)).call().content();
	}

	@Override
	public Flux<String> streamChat(String query) {
		return this.chatClient.prompt().system(system -> system.text(systemMessage))
